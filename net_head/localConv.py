...
kernel_localConv_updateOutput = '''
    extern "C" __global__ void kernel_localConv_updateOutput(
        const int n,
        const float* input,
...
kernel_localConv_updateGradVertical = '''
    extern "C" __global__ void kernel_localConv_updateGradVertical(
        const int n,
...
kernel_localConv_updateGradHorizontal = '''
    extern "C" __global__ void kernel_localConv_updateGradHorizontal(
        const int n,
        const float* gradLoss,
        const float* input,
        const float* vertical,
...
class FunctionLocalConvKernel(torch.autograd.Function):
    ...
    def forward(ctx, input, vertical, horizontal):
        ...
        if input.is_cuda == True:
            ...
            n = output.nelement()
            cupy_launch('kernel_localConv_updateOutput', cupy_kernel('kernel_localConv_updateOutput',{
                'input': input,
                'vertical': vertical,
    ...
    def backward(ctx, gradOutput):
        ...
        intInputHeight = input.size(2)
        intInputWidth = input.size(3)
        intFilterSize = vertical.size(1)
        intOutputHeight = vertical.size(2)
        intOutputWidth = vertical.size(3)
        ...
        if input.is_cuda == True:
            ...
            cupy_launch('kernel_localConv_updateGradVertical', cupy_kernel('kernel_localConv_updateGradVertical', {
                'gradLoss': gradOutput,
                'input': input,
                'horizontal': horizontal,
            ...
            cupy_launch('kernel_localConv_updateGradHorizontal', cupy_kernel('kernel_localConv_updateGradHorizontal', {
                'gradLoss': gradOutput,
                'input': input,
                'vertical': vertical,
            ...
            raise NotImplementedError()
        return gradInput, gradVertical, gradHorizontal,None
